...
class UserProfile(BaseModel):
    name: Annotated[
        str, Field(max_length=50, title="Name of the user", description="Description ")
    ]  # name : str = Field(max_lenght=50)  -> example for using field function, remaining all are data validation using Annotated and Field
    age: Annotated[int, Field(gt=0, lt=100, strict=True)]
    gender: Annotated[str, Field(max_length=1, default="Others", description="M or F")]
    country: Annotated[
        Optional[str], Field(default=None, description="Name your country")
    ]
    hobbies: Annotated[
        Optional[List[str]],
        Field(default=None, max_length=10, description="Any hobbies...?"),
    ]
    contact_number: Annotated[
        Optional[str], Field(default=None, description="Enter your mobiole number")
    ]
...
profile = {
    ...
    "hobbies": [
        ...
        "2",
        "3",
    ],  # the keys in the dict must be same as the variables set in the pydantic model
...
def reading_user_profile(p: UserProfile):
    ...
    # or just use print(p.model_dump()) to print complete raw data
